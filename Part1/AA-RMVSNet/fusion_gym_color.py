...
#             color = np.ones_like(color)
            ...
            for j in range(len(colors)):
                ...
                if np.all(colors[j] == np.array([192, 64, 64])):
                    color[target_indices] = np.array([0, 64, 192], dtype=np.float32) / 255.
                else:
...
if __name__ == '__main__':
    ...
    for scan in scans:
        ...
        if args.test_dataset == 'dtu':
            ...
            photo_threshold = 0.2
            filter_depth(scan_folder, out_folder, os.path.join(args.outdir, scan + '_color.ply'), photo_threshold)
