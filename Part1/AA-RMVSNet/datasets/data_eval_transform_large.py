...
class MVSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            intrinsics, extrinsics, depth_min, depth_interval, depth_end_ori = self.read_cam_file(proj_mat_filename)
            ...
            if i == 0:  # reference view
                if self.inverse_depth:  # slice inverse depth
                    print('inverse depth')
                    # depth_end = depth_interval * self.ndepths + depth_min # wether depth_end is this
                    depth_end = depth_end_ori - depth_interval / self.interval_scale
                    ...
                    depth_values2 = np.linspace(0, depth_min, 128, endpoint=False)
                    depth_values2 = depth_values.astype(np.float32)
                    depth_values = np.concatenate((depth_values2, depth_values), axis=0)

                else:
                    depth_values = np.arange(depth_min, depth_interval * self.ndepths + depth_min, depth_interval,
                                             dtype=np.float32)  # the set is [)
                    depth_end = depth_interval * self.ndepths + depth_min  # depth_values = np.arange(depth_min, depth_interval * (self.ndepths - 0.5) + depth_min, depth_interval,  #                          dtype=np.float32)
        ...
        return {"imgs": croped_imgs, "proj_matrices": new_proj_matrices, "depth_values": depth_values,
