...
class BlendedMVSDataset(Dataset):
    def __init__(self, root_dir, split, n_views=3, levels=3, depth_interval=192.0, img_wh=(768, 576)):
        """
        img_wh should be set to a tuple ex: (1152, 864) to enable test mode!
        """
        self.root_dir = root_dir
        self.split = split
        assert self.split in ['train', 'val', 'all'], 'split must be either "train", "val" or "all"!'
        self.img_wh = img_wh
    ...
    def build_metas(self):
        ...
        if self.split == 'train':
            list_txt = os.path.join(self.root_dir, 'train.txt')
        elif self.split == 'val':
            list_txt = os.path.join(self.root_dir, 'val.txt')
        else:
...
#         if self.root_dir.endswith('dataset_low_res') or self.root_dir.endswith('dataset_low_res/'):
        img_w, img_h = 768, 576
#         else:
#             img_w, img_h = 2048, 1536
    ...
    def read_depth_and_mask(self, scan, filename, depth_min):
        ...
        masks = {"level_0": torch.BoolTensor(depth_0 > depth_min), "level_1": torch.BoolTensor(depth_1 > depth_min),
    ...
    def define_transforms(self):
        if self.split == 'train':  # you can add augmentation here
            self.transform = T.Compose([T.ColorJitter(brightness=0.25, contrast=0.5), T.ToTensor(),
