...
class CustomDataset(Dataset):
    def __init__(self, root_dir, split, n_views=3, levels=3, depth_interval=192.0, img_wh=(768, 576)):
        ...
        self.split = split
        assert self.split in ['dongbeiya', 'gym'], 'split must be either "dongbeiya" or "gym" !'
        self.img_wh = img_wh
    ...
    def build_metas(self):
        ...
        list_txt = os.path.join(self.root_dir, self.split + '.txt')
    ...
    def read_depth_and_mask(self, scan, filename, depth_min):
        ...
        depths = {
            "level_0": torch.FloatTensor(depth_0),
            "level_1": torch.FloatTensor(depth_1),
            "level_2": torch.FloatTensor(depth_2)
        }

        masks = {
            "level_0": torch.BoolTensor(depth_0 > depth_min),
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            img_filename = os.path.join(self.root_dir, f'{scan}/images/{vid:08d}.jpg')
