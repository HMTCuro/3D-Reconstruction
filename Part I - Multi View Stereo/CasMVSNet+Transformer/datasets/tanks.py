...
class TanksDataset(Dataset):
    def __init__(self, root_dir, split='intermediate', n_views=3, levels=3, depth_interval=-1, img_wh=(1152, 864)):
        """
    ...
    def build_metas(self):
        ...
        if self.split == 'intermediate':
            ...
            self.image_sizes = {'Family': (1920, 1080), 'Francis': (1920, 1080), 'Horse': (1920, 1080),
                                'Lighthouse': (2048, 1080), 'M60': (2048, 1080), 'Panther': (2048, 1080),
                                'Playground': (1920, 1080), 'Train': (1920, 1080)}
            self.depth_interval = {'Family': 2.5e-3, 'Francis': 1e-2, 'Horse': 1.5e-3, 'Lighthouse': 1.5e-2,
                                   'M60': 5e-3, 'Panther': 5e-3, 'Playground': 7e-3,
                                   'Train': 5e-3}  # depth interval for each scan (hand tuned)
        elif self.split == 'advanced':
            self.scans = ['Auditorium', 'Ballroom', 'Courtroom', 'Museum', 'Palace', 'Temple']
            self.image_sizes = {'Auditorium': (1920, 1080), 'Ballroom': (1920, 1080), 'Courtroom': (1920, 1080),
                                'Museum': (1920, 1080), 'Palace': (1920, 1080), 'Temple': (1920, 1080)}
            self.depth_interval = {'Auditorium': 3e-2, 'Ballroom': 2e-2, 'Courtroom': 2e-2, 'Museum': 2e-2,
                                   'Palace': 1e-2, 'Temple': 1e-2}  # depth interval for each scan (hand tuned)
