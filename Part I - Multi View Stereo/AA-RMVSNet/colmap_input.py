...
CAMERA_MODELS = {CameraModel(model_id=0, model_name="SIMPLE_PINHOLE", num_params=3),
...
def read_cameras_binary(path_to_model_file):
    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for camera_line_index in range(num_cameras):
            ...
            cameras[camera_id] = Camera(id=camera_id, model=model_name, width=width, height=height,
...
def read_images_binary(path_to_model_file):
    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for image_index in range(num_reg_images):
            ...
            images[image_id] = Image(id=image_id, qvec=qvec, tvec=tvec, camera_id=camera_id, name=image_name, xys=xys,
    ...
    with open(path, "r") as fid:
        while True:
            ...
            if len(line) > 0 and line[0] != "#":
                ...
                points3D[point3D_id] = Point3D(id=point3D_id, xyz=xyz, rgb=rgb, error=error, image_ids=image_ids,
...
def qvec2rotmat(qvec):
    return np.array([[1 - 2 * qvec[2] ** 2 - 2 * qvec[3] ** 2, 2 * qvec[1] * qvec[2] - 2 * qvec[0] * qvec[3],
...
def rotmat2qvec(R):
    ...
    K = np.array(
        [[Rxx - Ryy - Rzz, 0, 0, 0], [Ryx + Rxy, Ryy - Rxx - Rzz, 0, 0], [Rzx + Rxz, Rzy + Ryz, Rzz - Rxx - Ryy, 0],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_d', type=int, default=1024)
    parser.add_argument('--interval_scale', type=float, default=0.2)
    ...
    image_dir = os.path.join(args.folder, 'dby')
    model_dir = os.path.join(args.folder, 'sparse1')
    # model_dir = os.path.join(args.folder, 'dslr_calibration_undistorted')
    cam_dir = os.path.join(args.folder, 'cams')
    ...
    # 选择相机模型
    param_type = {'SIMPLE_PINHOLE': ['f', 'cx', 'cy'], 'PINHOLE': ['fx', 'fy', 'cx', 'cy'],
        'SIMPLE_RADIAL': ['f', 'cx', 'cy', 'k'], 'SIMPLE_RADIAL_FISHEYE': ['f', 'cx', 'cy', 'k'],
        'RADIAL': ['f', 'cx', 'cy', 'k1', 'k2'], 'RADIAL_FISHEYE': ['f', 'cx', 'cy', 'k1', 'k2'],
    ...
    for i in range(num_images):
        ...
        depth_ranges[i + 1] = (depth_min, depth_interval)
        ...
        for pid in id_intersect:
            ...
            theta = (180 / np.pi) * np.arccos(
                np.dot(cam_center_i - p, cam_center_j - p) / np.linalg.norm(cam_center_i - p) / np.linalg.norm(
                    cam_center_j - p))
            s += np.exp(-(theta - args.theta0) * (theta - args.theta0) / (
